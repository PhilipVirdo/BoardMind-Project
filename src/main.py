...
def mcts(board, depth, num_simulations, maximizing_player, model):
    ...
    with ThreadPoolExecutor() as executor:
        ...
        for move in sorted_moves:
            ...
            if board.castling_rights and chess.BB_H8 or board.castling_rights and chess.BB_A8:
                value += 100
            board.pop()
...
def train_model(model, trainloader, num_epochs, model_path):
    model.train()
...
def main():
    ...
    pgn_folder = pgn_folder = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..','data', 'external', 'ChessCheckMateDatabase')
    ...
    #for positions, targets in dataset.load_pgn_files(pgn_files, chunk_size):
        ...
        #train_model(model, trainloader, num_epochs, model_path)
    #torch.save(model.state_dict(), model_path)
    ####
